...
void dispatcher(auto& queue, const auto& config, auto num_commands, auto& command_result_vec, auto& latency_result_vec)
{
    ...
    while (processed < num_commands) {
        ...
        uint64_t latency_ns = static_cast<uint64_t>(latency_res.latency_in_seconds * 1e9);
...
CommandMetrics compute_metrics(const MatmulCommand& cmd, const LatencyResult& res, const AcceleratorConfig& config)
{
    uint64_t flops = 2 * cmd.m * cmd.k * cmd.n;
    uint64_t bytes = config.bytes_per_element * ((cmd.m * cmd.k) + (cmd.k * cmd.n) + (cmd.m * cmd.n));
...
void write_results(
    const std::vector<MatmulCommand>& commands,
    ...
    const char* filename)
{
    ...
    for (size_t i = 0; i < commands.size(); ++i) {
        ...
        // stdout
        std::cout << std::fixed << std::setprecision(2) << "(" << cmd.m << "," << cmd.k << "," << cmd.n << ")"
                  << " | FLOPs=" << metrics.flops << " | bytes=" << metrics.bytes
                  << " | AI=" << metrics.arithmetic_intensity << " | bound=" << res.bound_type
                  << " | latency_s=" << std::scientific << res.latency_in_seconds << " | cycles=" << std::fixed
                  << metrics.cycles << '\n';

        // CSV
        out << cmd.m << ',' << cmd.k << ',' << cmd.n << ',' << metrics.flops << ',' << metrics.bytes << ','
            << std::fixed << std::setprecision(4) << metrics.arithmetic_intensity << ',' << std::quoted(res.bound_type)
            << ',' << std::scientific << std::setprecision(6) << res.latency_in_seconds << ',' << std::fixed
            << std::setprecision(0) << metrics.cycles << '\n';
    }
...
AggregateStats compute_aggregates(const std::vector<LatencyResult>& results, const AcceleratorConfig& config)
{
    std::vector<uint64_t> cycle_latencies;
    cycle_latencies.reserve(results.size());
    for (const auto& res : results) {
        cycle_latencies.push_back(static_cast<uint64_t>(res.latency_in_seconds * config.clock_freq));
    }
...
int main()
{
    AcceleratorConfig config {
        .flops_per_second = 1e12, .bandwidth_bytes_per_second = 1e12, .bytes_per_element = 2.0, .clock_freq = 1e9
    };

    std::vector<MatmulCommand> tests = { { 2, 2, 2 },    { 5, 5, 5 },    { 10, 10, 10 },   { 20, 20, 20 },
    ...
    std::thread dispatch([&]() { dispatcher(q, config, num_commands, command_result_vec, latency_result_vec); });
    prod.join();
    dispatch.join();
